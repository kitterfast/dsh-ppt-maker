	...
	factory: (require) => {
		...
		function kickoff(useAi) {
			...
			return [
				...
				nDeliver + ". **迭代方式（上一轮就是这里翻的车）**：",
				"   - 你可以也应该自己开浏览器看效果，但按文档 §0.5.2 的**窗口协议**：常驻窗口/标签 **≤ 2**" +
				"（当前轮 + 上一轮，供前后对比），开新一轮时**复用同一扇窗**、**把更早的关掉**，不要一直堆；" +
				"复核一律用专属 profile，不要占用我日常在用的浏览器；",
				"   - 按文档 §0.6 **批量迭代**：一轮 = **看全册 → 一次列全问题 → 一次改完 → 只验一次**；" +
				"**严禁**看一页改一页、严禁改一处就全量重跑；",
				"   - **每轮必须有净变化**（改了哪几页的哪几件事、销掉几项）。**净变化为零就立刻停下**，" +
				"把剩余清单交给我决定，不要继续空转；",
				"   - **修改边界**（文档 §0.6.1）开工时就写给我：清单清空 / 连续一轮零新增 / 验收通过 / HTML 阶段**最多 3 轮**；",
				"   - 只有**最终交付物**打印绝对路径 + `present`，中间产物不登记；`build/<稿名>.html` 是**唯一**权威文件，**原地改**。",
				nRules + ". **转 PPTX 不要手写转换器**：直接跑本插件自带的 `scripts/deck/` 三条命令（文档 C7-1）。",
				"   上一轮现场写了 48 个一次性脚本、转了 1 小时 44 分，最后仍然：没有动画、翻页按钮被烤进每一页、文字错版。",
				(nRules + 1) + ". **先探针后全量**：任何「每页都跑一遍」的动作先拿 1 页验证（例如 `deck-render.mjs … --only=6`），通过了再做全篇；未改动的页要用缓存，不许全量重跑。",
				(nRules + 2) + ". 全程守 §四 硬规则；每个阶段做完停下等我确认；同一条命令被拒**不要原样重试**，连续 2 次被拒就停下来告诉我。",
				"",
