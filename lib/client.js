	...
	factory: (require) => {
		...
		function kickoff(useAi) {
			...
			const envCheck = useAi
				? [
					"2. **开工第二件事：自己跑一次环境自检，全程不要让我做任何事。** 执行：",
					"   `powershell -NoProfile -ExecutionPolicy Bypass -File \"%USERPROFILE%\\.dsh\\local-plugins\\dsh-ppt-maker\\scripts\\check-env.ps1\"`",
					"   它逐项检查 Node / arkcli / dsh-chrome-cdp / 专属自动化浏览器 / 火山登录态，**缺什么自己装**（全局装 arkcli、`dsh plugin add`、按需拉起浏览器、跑 0 手动登录），并把结论打进就绪报告。",
